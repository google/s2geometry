...
struct ShapeEdge {
 public:
  ShapeEdge() {}
  ShapeEdge(const S2Shape& shape, std::int32_t edge_id);
  ShapeEdge(std::int32_t shape_id, std::int32_t edge_id, const S2Shape::Edge& edge);
...
inline ShapeEdge::ShapeEdge(const S2Shape& shape, std::int32_t edge_id)
    : ShapeEdge(shape.id(), edge_id, shape.edge(edge_id)) {
}

inline ShapeEdge::ShapeEdge(std::int32_t shape_id, std::int32_t edge_id,
