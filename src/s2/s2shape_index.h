...
class S2ClippedShape {
 ...
 private:
  ...
  void Init(std::int32_t shape_id, std::int32_t num_edges);
  void Destruct();
  ...
  std::int32_t shape_id_;
  std::uint32_t contains_center_ : 1;  // shape contains the cell center
  std::uint32_t num_edges_ : 31;

  // If there are more than two edges, this field holds a pointer.
  // Otherwise it holds an array of edge ids.
  union {
    std::int32_t* edges_;  // Owned by the containing S2ShapeIndexCell.
...
inline void S2ClippedShape::Init(std::int32_t shape_id, std::int32_t num_edges) {
  shape_id_ = shape_id;
  num_edges_ = num_edges;
  contains_center_ = false;
  if (!is_inline()) {
    edges_ = new std::int32_t[num_edges];
  }
}
