...
class S2VertexIdLaxLoopShape : public S2Shape {
 public:
  ...
  explicit S2VertexIdLaxLoopShape(const std::vector<std::int32_t>& vertex_ids,
                                  const S2Point* vertex_array);

  // Initializes the shape from the given vertex array and indices.
  // "vertex_ids" is a vector of indices into "vertex_array".
  void Init(const std::vector<std::int32_t>& vertex_ids,
            const S2Point* vertex_array);

  // Returns the number of vertices in the loop.
  int num_vertices() const { return num_vertices_; }
  std::int32_t vertex_id(int i) const { return vertex_ids_[i]; }
 ...
 private:
  std::int32_t num_vertices_;
  std::unique_ptr<std::int32_t[]> vertex_ids_;
