...
TEST(S2EarthTest, TestAngleConversion) {
  ...
  ASSERT_FLOAT_EQ(util::units::Meters(
                      S2Earth::ToDistance(S1Angle::Radians(2))).value(),
                  2 * S2Earth::RadiusMeters());
  ASSERT_DOUBLE_EQ(S2Earth::ToRadians(util::units::Meters(
      S2Earth::RadiusMeters())), 1);
...
TEST(S2EarthTest, TestToLongitudeRadians) {
  const util::units::Meters earth_radius = util::units::Meters(
  ...
  ASSERT_DOUBLE_EQ(S2Earth::ToLongitudeRadians(earth_radius, M_PI_2),
                   M_PI * 2);
...
TEST(S2EarthTest, TestGetDistance) {
  ...
  ASSERT_FLOAT_EQ(util::units::Miles(
                      S2Earth::GetDistance(north, south)).value(),
  ...
  ASSERT_FLOAT_EQ(util::units::Feet(S2Earth::GetDistance(
                           S2LatLng::FromDegrees(0, -90),
                           S2LatLng::FromDegrees(-90, -38))).value(),
