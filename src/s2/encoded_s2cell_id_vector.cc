...
void EncodeS2CellIdVector(Span<const S2CellId> v, Encoder* encoder) {
  ...
  std::uint64_t v_or = 0, v_and = ~0ULL, v_min = ~0ULL, v_max = 0;
  for (auto cellid : v) {
    v_or |= cellid.id();
    v_and &= cellid.id();
  ...
  std::uint64_t e_base = 0;        // Base value.
  ...
  if (v_or > 0) {
    ...
    std::uint64_t e_bytes = ~0ULL;  // Best encoding size so far.
    for (int len = 0; len < 8; ++len) {
      ...
      std::uint64_t t_base = v_min & ~(~0ULL >> (8 * len));
      int t_max_delta_msb =
          max(0, Bits::Log2Floor64((v_max - t_base) >> e_shift));
      std::uint64_t t_bytes = len + v.size() * ((t_max_delta_msb >> 3) + 1);
      if (t_bytes < e_bytes) {
        e_base = t_base;
        e_base_len = len;
  ...
  std::uint64_t base_bytes = e_base >> (64 - 8 * max(1, e_base_len));
  EncodeUintWithLength<std::uint64_t>(base_bytes, e_base_len, encoder);

  // Finally, encode the vector of deltas.
  vector<std::uint64_t> deltas;
  deltas.reserve(v.size());
  for (auto cellid : v) {
    deltas.push_back((cellid.id() - e_base) >> e_shift);
  }
  EncodeUintVector<std::uint64_t>(deltas, encoder);
}
