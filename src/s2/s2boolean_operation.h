...
class S2BooleanOperation {
 public:
  ...
  class SourceId {
   public:
    SourceId();
    SourceId(int region_id, std::int32_t shape_id, std::int32_t edge_id);
    explicit SourceId(std::int32_t special_edge_id);
   ...
   private:
    std::uint32_t region_id_ : 1;
    std::uint32_t shape_id_ : 31;
    std::int32_t edge_id_;
  };
  ...
  class Options {
   public:
    ...
    //
    // for (std::int32_t label : label_set_lexicon.id_set(label_set_id)) {
...
inline S2BooleanOperation::SourceId::SourceId(
    int region_id, std::int32_t shape_id, std::int32_t edge_id)
