...
uint64_t InterleaveUint32(const uint32_t val0, const uint32_t val1) {
  return (static_cast<uint64_t>(kInterleaveLut[val0 & 0xff])) |
...
inline uint8_t ExtractEvenBits(uint16_t bits) {
  bits &= 0x5555;
  bits |= bits >> 1;
  bits &= 0x3333;
  bits |= bits >> 2;
...
// Extracting the even bits (bit 0, 2, ...).
inline uint16_t ExtractEvenBits(uint32_t bits) {
  bits &= 0x55555555;
  bits |= bits >> 1;
  bits &= 0x33333333;
  bits |= bits >> 2;
...
// Extracting the even bits (bit 0, 2, ...).
inline uint32_t ExtractEvenBits(uint64_t bits) {
