...
static const std::uint16_t kInterleaveLut[256] = {
...
std::uint16_t InterleaveUint8(const std::uint8_t val0, const std::uint8_t val1) {
  return kInterleaveLut[val0] | (kInterleaveLut[val1] << 1);
}

std::uint32_t InterleaveUint16(const std::uint16_t val0, const std::uint16_t val1) {
...
std::uint64_t InterleaveUint32(const std::uint32_t val0, const std::uint32_t val1) {
  return
      (static_cast<std::uint64_t>(kInterleaveLut[val0 & 0xff])) |
      (static_cast<std::uint64_t>(kInterleaveLut[(val0 >> 8) & 0xff]) << 16) |
      (static_cast<std::uint64_t>(kInterleaveLut[(val0 >> 16) & 0xff]) << 32) |
      (static_cast<std::uint64_t>(kInterleaveLut[val0 >> 24]) << 48) |
      (static_cast<std::uint64_t>(kInterleaveLut[val1 & 0xff]) << 1) |
      (static_cast<std::uint64_t>(kInterleaveLut[(val1 >> 8) & 0xff]) << 17) |
      (static_cast<std::uint64_t>(kInterleaveLut[(val1 >> 16) & 0xff]) << 33) |
      (static_cast<std::uint64_t>(kInterleaveLut[val1 >> 24]) << 49);
}
...
//
static const std::uint8_t kDeinterleaveLut[256] = {
...
void DeinterleaveUint8(std::uint16_t val, std::uint8_t *val0, std::uint8_t *val1) {
...
template<int kShift>
static std::uint64_t SplitFor3(std::uint8_t x) {
  return
...
static inline std::uint8_t UnsplitFor3(std::uint32_t x) {
...
void DeinterleaveUint8(std::uint32_t x, std::uint8_t* a, std::uint8_t* b, std::uint8_t* c) {
  *a = UnsplitFor3(x);
  *b = UnsplitFor3(x >> 1);
  *c = UnsplitFor3(x >> 2);
}
