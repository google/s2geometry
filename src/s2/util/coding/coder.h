...
class Encoder {
 public:
  ...
  void put_varint32(std::uint32_t v);
  void put_varint32_inline(std::uint32_t v);
  void put_varint64(std::uint64_t v);
  static int varint32_length(std::uint32_t v);  // Length of var encoding of "v"
  static int varint64_length(std::uint64_t v);  // Length of var encoding of "v"
...
class Decoder {
 public:
  ...
  bool get_varint32(std::uint32_t* v);
  bool get_varint64(std::uint64_t* v);
...
inline void Encoder::put_varint32(std::uint32_t v) {
  buf_ = reinterpret_cast<unsigned char*>
         (Varint::Encode32(reinterpret_cast<char*>(buf_), v));
}

inline void Encoder::put_varint32_inline(std::uint32_t v) {
  buf_ = reinterpret_cast<unsigned char*>
         (Varint::Encode32Inline(reinterpret_cast<char*>(buf_), v));
}

inline void Encoder::put_varint64(std::uint64_t v) {
  buf_ = reinterpret_cast<unsigned char*>
...
template <size_t N>
void CopyAndAdvance(const std::uint8_t** src, std::uint8_t** dst) {
  memcpy(*dst, *src, N);
  *dst += N;
  *src += N;
}
...
inline void Encoder::put16(std::uint16_t v) {
  S2_DCHECK_GE(avail(), sizeof(v));
  LittleEndian::Store16(buf_, v);
  buf_ += sizeof(v);
}

inline void Encoder::put32(std::uint32_t v) {
  S2_DCHECK_GE(avail(), sizeof(v));
  LittleEndian::Store32(buf_, v);
  buf_ += sizeof(v);
}

inline void Encoder::put64(std::uint64_t v) {
  S2_DCHECK_GE(avail(), sizeof(v));
...
inline void Encoder::putfloat(float f) {
  put32(absl::bit_cast<std::uint32_t>(f));
}

inline void Encoder::putdouble(double d) {
  put64(absl::bit_cast<std::uint64_t>(d));
}
...
inline std::uint16_t Decoder::get16() {
  const std::uint16_t v = LittleEndian::Load16(buf_);
  buf_ += sizeof(v);
  return v;
}

inline std::uint32_t Decoder::get32() {
  const std::uint32_t v = LittleEndian::Load32(buf_);
  buf_ += sizeof(v);
  return v;
}

inline std::uint64_t Decoder::get64() {
  const std::uint64_t v = LittleEndian::Load64(buf_);
  buf_ += sizeof(v);
  return v;
}
...
inline bool Decoder::get_varint32(std::uint32_t* v) {
  const char* const r =
...
inline bool Decoder::get_varint64(std::uint64_t* v) {
  const char* const r =
