...
class Varint {
 public:
  // Maximum lengths of varint encoding of std::uint32_t and std::uint64_t
  ...
  static const char* Parse32(const char* ptr, std::uint32_t* OUTPUT);
  static const char* Parse64(const char* ptr, std::uint64_t* OUTPUT);

  // A fully inlined version of Parse32: useful in the most time critical
  // routines, but its code size is large
  static const char* Parse32Inline(const char* ptr, std::uint32_t* OUTPUT);
  ...
  static char* Encode32(char* ptr, std::uint32_t v);
  static char* Encode64(char* ptr, std::uint64_t v);

  // A fully inlined version of Encode32: useful in the most time critical
  // routines, but its code size is large
  static char* Encode32Inline(char* ptr, std::uint32_t v);

  // EFFECTS    Returns the encoding length of the specified value.
  static int Length32(std::uint32_t v);
  static int Length64(std::uint64_t v);

  // EFFECTS    Appends the varint representation of "value" to "*s".
  static void Append32(string* s, std::uint32_t value);
  static void Append64(string* s, std::uint64_t value);
  ...
  static void EncodeTwo32Values(string* s, std::uint32_t a, std::uint32_t b);
  ABSL_DEPRECATED("Use TwoValuesVarint::Decode32.")
  static const char* DecodeTwo32Values(const char* ptr, std::uint32_t* a, std::uint32_t* b);
  ABSL_DEPRECATED("Use TwoValuesVarint::Decode32WithLimit.")
  static const char* DecodeTwo32ValuesWithLimit(const char* ptr,
                                                const char* limit, std::uint32_t* a,
                                                std::uint32_t* b);
  ...
  //            goal is positive and fit into a signed std::int64_t.
  // EFFECTS    Returns a pointer just past last read byte.
  //            "out" stores the actual sum.
  static const char* FastDecodeDeltas(const char* ptr, std::int64_t goal, std::int64_t* out);

 private:
  static const char* Parse32FallbackInline(const char* p, std::uint32_t* val);
  static const char* Parse32Fallback(const char* p, std::uint32_t* val);
  static const char* Parse64Fallback(const char* p, std::uint64_t* val);

  static char* Encode32Fallback(char* ptr, std::uint32_t v);
  ...
  static void Append32Slow(string* s, std::uint32_t value);
  static void Append64Slow(string* s, std::uint64_t value);
...
inline const char* Varint::Parse32(const char* p, std::uint32_t* OUTPUT) {
...
inline const char* Varint::Parse32Inline(const char* p, std::uint32_t* OUTPUT) {
  // Fast path for inlining
  const unsigned char* ptr = reinterpret_cast<const unsigned char*>(p);
  std::uint32_t byte = *ptr;
  if (byte < 128) {
...
inline const char* Varint::Parse64(const char* p, std::uint64_t* OUTPUT) {
  const unsigned char* ptr = reinterpret_cast<const unsigned char*>(p);
  std::uint32_t byte = *ptr;
  if (byte < 128) {
    *OUTPUT = byte;
...
inline int Varint::Length32(std::uint32_t v) {
  // This computes value == 0 ? 1 : floor(log2(v)) / 7 + 1
  // Use an explicit multiplication to implement the divide of
  // a number in the 1..31 range.
  // Explicit OR 0x1 to handle v == 0.
  std::uint32_t log2value = Bits::Log2FloorNonZero(v | 0x1);
  return static_cast<int>((log2value * 9 + 73) / 64);
}

inline int Varint::Length64(std::uint64_t v) {
  // This computes value == 0 ? 1 : floor(log2(v)) / 7 + 1
  // Use an explicit multiplication to implement the divide of
  // a number in the 1..63 range.
  // Explicit OR 0x1 to handle v == 0.
  std::uint32_t log2value = Bits::Log2FloorNonZero64(v | 0x1);
  return static_cast<int>((log2value * 9 + 73) / 64);
}

inline void Varint::Append32(string* s, std::uint32_t value) {
...
inline void Varint::Append64(string* s, std::uint64_t value) {
  // Inline the fast-path for single-character output, but fall back to the .cc
  // file for the full version. The size<capacity check is so the compiler can
  // optimize out the string resize code.
  if (value < 128 && s->size() < s->capacity()) {
...
inline char* Varint::Encode32Inline(char* sptr, std::uint32_t v) {
  // Operate on characters as unsigneds
  std::uint8_t* ptr = reinterpret_cast<std::uint8_t*>(sptr);
  static const std::uint32_t B = 128;
  if (v < (1<<7)) {
    *(ptr++) = static_cast<std::uint8_t>(v);
  } else if (v < (1<<14)) {
    *(ptr++) = static_cast<std::uint8_t>(v | B);
    *(ptr++) = static_cast<std::uint8_t>(v>>7);
  } else if (v < (1<<21)) {
    *(ptr++) = static_cast<std::uint8_t>(v | B);
    *(ptr++) = static_cast<std::uint8_t>((v>>7) | B);
    *(ptr++) = static_cast<std::uint8_t>(v>>14);
  } else if (v < (1<<28)) {
    *(ptr++) = static_cast<std::uint8_t>(v | B);
    *(ptr++) = static_cast<std::uint8_t>((v>>7) | B);
    *(ptr++) = static_cast<std::uint8_t>((v>>14) | B);
    *(ptr++) = static_cast<std::uint8_t>(v>>21);
  } else {
    *(ptr++) = static_cast<std::uint8_t>(v | B);
    *(ptr++) = static_cast<std::uint8_t>((v>>7) | B);
    *(ptr++) = static_cast<std::uint8_t>((v>>14) | B);
    *(ptr++) = static_cast<std::uint8_t>((v>>21) | B);
    *(ptr++) = static_cast<std::uint8_t>(v>>28);
  }
...
inline const char* Varint::FastDecodeDeltas(const char* ptr,
                                            ...
                                            std::int64_t* out) {
  std::int64_t value;
  std::int64_t sum = - goal;
  std::int64_t shift = 0;
  // Make decoding faster by eliminating unpredictable branching.
  do {
    value = static_cast<std::int8_t>(*ptr++);  // sign extend one byte of data
