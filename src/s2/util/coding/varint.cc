...
char* Varint::Encode32(char* sptr, std::uint32_t v) {
  return Encode32Inline(sptr, v);
}

char* Varint::Encode64(char* sptr, std::uint64_t v) {
  if (v < (1u << 28)) {
    ...
    const std::uint32_t x32 = v | (1 << 7) | (1 << 21);
    const std::uint32_t y32 = v | (1 << 14) | (1 << 28);
    *(ptr++) = x32;
    *(ptr++) = y32 >> 7;
    *(ptr++) = x32 >> 14;
    *(ptr++) = y32 >> 21;
...
const char* Varint::Parse32Fallback(const char* ptr, std::uint32_t* OUTPUT) {
  return Parse32FallbackInline(ptr, OUTPUT);
}

const char* Varint::Parse64Fallback(const char* p, std::uint64_t* OUTPUT) {
  ...
  std::uint64_t byte, res1, res2 = 0, res3 = 0;
  byte = *(ptr++); res1 = byte;
  ...
  *OUTPUT = res1 + ((res2 - 1) << 28) + ((res3 - 1) << 56);
  return reinterpret_cast<const char*>(ptr);
#else
  std::uint32_t byte, res1, res2=0, res3=0;
 ...
 done2:
  assert(res3 == 0);
  *OUTPUT = res1 | (std::uint64_t(res2) << 28);
  return reinterpret_cast<const char*>(ptr);

 done3:
  *OUTPUT = res1 | (std::uint64_t(res2) << 28) | (std::uint64_t(res3) << 56);
  return reinterpret_cast<const char*>(ptr);
#endif
}
...
void Varint::Append32Slow(string* s, std::uint32_t value) {
  const size_t start = s->size();
  s->resize(start + Varint::Length32(value));
  Varint::Encode32(&((*s)[start]), value);
}

void Varint::Append64Slow(string* s, std::uint64_t value) {
  const size_t start = s->size();
