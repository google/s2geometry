...
S2VertexIdLaxLoopShape::S2VertexIdLaxLoopShape(
    const std::vector<std::int32_t>& vertex_ids, const S2Point* vertex_array) {
  Init(vertex_ids, vertex_array);
}

void S2VertexIdLaxLoopShape::Init(const std::vector<std::int32_t>& vertex_ids,
                           const S2Point* vertex_array) {
  num_vertices_ = vertex_ids.size();
  vertex_ids_.reset(new std::int32_t[num_vertices_]);
