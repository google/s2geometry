...
void S2ShapeIndexCell::Encode(int num_shape_ids, Encoder* encoder) const {
  ...
  if (num_shape_ids == 1) {
    ...
    if (n >= 2 && n <= 17 && clipped.edge(n - 1) - clipped.edge(0) == n - 1) {
      ...
      //           bits 6+: edge_id
      encoder->put_varint64(static_cast<std::uint64_t>(clipped.edge(0)) << 6 |
      ...
      //           bits 3+: edge_id
      encoder->put_varint64(static_cast<std::uint64_t>(clipped.edge(0)) << 3 |
      ...
      //           bit 2: contains_center
      //           bits 3+: num_edges
      encoder->put_varint64(static_cast<std::uint64_t>(n) << 3 |
