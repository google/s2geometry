...
TEST(CompareDistances, Consistency) {
  ...
  for (int iter = 0; iter < FLAGS_consistency_iters; ++iter) {
    ...
    if (rnd.OneIn(2)) r = S1Angle::Radians(M_PI_2) + r;
    S2Point a = S2EdgeUtil::InterpolateAtDistance(r, x, dir);
    S2Point b = S2EdgeUtil::InterpolateAtDistance(r, x, -dir);
...
TEST(CompareEdgeDistance, Consistency) {
  ...
  for (int iter = 0; iter < FLAGS_consistency_iters; ++iter) {
    ...
    if (rnd.OneIn(2)) r = S1Angle::Radians(M_PI_2) - r;
    S2Point x = S2EdgeUtil::InterpolateAtDistance(r, a, n);
    if (rnd.OneIn(5)) {
...
TEST(CompareEdgeDirections, Consistency) {
  ...
  for (int iter = 0; iter < FLAGS_consistency_iters; ++iter) {
    ...
    S2Point a1 = S2EdgeUtil::InterpolateAtDistance(a_len, a0, ChoosePoint());
    ...
    S2Point b1 = S2EdgeUtil::InterpolateAtDistance(b_len, b0, a_norm);
...
TEST(VoronoiSiteExclusion, Consistency) {
  ...
  for (int iter = 0; iter < FLAGS_consistency_iters; ++iter) {
    ...
    S2Point a = S2EdgeUtil::InterpolateAtDistance(r1, p, ChoosePoint());
    S2Point b = S2EdgeUtil::InterpolateAtDistance(r1, p, ChoosePoint());
