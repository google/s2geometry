...
std::int32_t strto32_adapter(const char *nptr, char **endptr, int base) {
  ...
  } else if (errno == 0 && result < std::numeric_limits<std::int32_t>::min()) {
    errno = ERANGE;
    return std::numeric_limits<std::int32_t>::min();
  } else if (errno == 0 && result > std::numeric_limits<std::int32_t>::max()) {
    errno = ERANGE;
    return std::numeric_limits<std::int32_t>::max();
  }
  if (errno == 0)
...
std::uint32_t strtou32_adapter(const char *nptr, char **endptr, int base) {
  ...
  } else if (errno == 0 && result > std::numeric_limits<std::uint32_t>::max()) {
    errno = ERANGE;
    return std::numeric_limits<std::uint32_t>::max();
  }
  if (errno == 0)
