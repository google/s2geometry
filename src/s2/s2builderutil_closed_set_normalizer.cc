...
ClosedSetNormalizer::ClosedSetNormalizer(
  ...
  ABSL_DCHECK_EQ(graph_options_out_.size(), 3);
  ABSL_DCHECK(graph_options_out_[0].edge_type() == EdgeType::DIRECTED);
  ABSL_DCHECK(graph_options_out_[2].edge_type() == EdgeType::DIRECTED);
  ...
  ABSL_DCHECK(graph_options_out_[1].sibling_pairs() != SiblingPairs::CREATE);
  ABSL_DCHECK(graph_options_out_[1].sibling_pairs() != SiblingPairs::REQUIRE);
...
class NormalizeClosedSetImpl {
 ...
 private:
  NormalizeClosedSetImpl(LayerVector output_layers,
    ...
    ABSL_DCHECK_EQ(3, output_layers_.size());
  }
