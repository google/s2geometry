...
void VerifyS2PointVectorLayerResults(
    ...
    const vector<vector<std::int32_t>>& expected_labels) {
  ...
  for (int i = 0; i < output.size(); ++i) {
    ...
    int k = 0;
    for (std::int32_t label : label_set_lexicon.id_set(label_set_ids[i])) {
...
TEST(S2PointVectorLayer, MergeDuplicates) {
  ...
  vector<vector<std::int32_t>> expected_labels = {{1, 2}, {1}, {2}, {2}, {}};
