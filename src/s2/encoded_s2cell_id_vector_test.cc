...
// Like the above, but accepts a vector<std::uint64_t> rather than a vector<S2CellId>.
void TestEncodedS2CellIdVector(const vector<std::uint64_t>& raw_expected,
                               size_t expected_bytes) {
  vector<S2CellId> expected;
  for (std::uint64_t raw_id : raw_expected) {
