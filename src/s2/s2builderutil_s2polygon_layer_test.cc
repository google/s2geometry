...
static void TestEdgeLabels(EdgeType edge_type) {
  ...
  for (int i = 0; i < expected_loop_sizes.size(); ++i) {
    ...
    for (int j = 0; j < label_set_ids[i].size(); ++j) {
      ...
      const set<std::int32_t>& expected_labels = edge_label_map[key];
