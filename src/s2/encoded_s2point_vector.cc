...
inline std::uint64_t InterleaveUint32BitPairs(const std::uint32_t val0, const std::uint32_t val1) {
  std::uint64_t v0 = val0, v1 = val1;
...
inline void DeinterleaveUint32BitPairs(std::uint64_t code,
                                       std::uint32_t *val0, std::uint32_t *val1) {
  std::uint64_t v0 = code, v1 = code >> 2;
...
static const std::uint8_t kEncodingFormatMask = (1 << kEncodingFormatBits) - 1;
...
void EncodeS2PointVectorFast(Span<const S2Point> points, Encoder* encoder) {
  ...
  std::uint64_t size_format = (points.size() << kEncodingFormatBits |
...
struct CellPoint {
  ...
  CellPoint(int level, int face, std::uint32_t si, std::uint32_t ti)
    : level(level), face(face), si(si), ti(ti) {}

  std::int8_t level, face;
  std::uint32_t si, ti;
};
...
constexpr std::uint64_t kException = ~0ULL;
...
inline std::uint64_t BitMask(int n) {
...
vector<std::uint64_t> ConvertCellsToValues(const vector<CellPoint>& cell_points,
                                    int level, bool* have_exceptions);
std::uint64_t ChooseBase(const vector<std::uint64_t>& values, int level, bool have_exceptions,
                  int* base_bits);
BlockCode GetBlockCode(Span<const std::uint64_t> values, std::uint64_t base,
...
void EncodeS2PointVectorCompact(Span<const S2Point> points, Encoder* encoder) {
  ...
  bool have_exceptions;
  vector<std::uint64_t> values = ConvertCellsToValues(cell_points, level,
                                               &have_exceptions);

  // 3. Choose the global encoding parameter "base" (consisting of the bit
  // prefix shared by all values to be encoded).
  int base_bits;
  std::uint64_t base = ChooseBase(values, level, have_exceptions, &base_bits);
  ...
  std::uint64_t offset_bytes_sum = 0;
  std::uint64_t delta_nibbles_sum = 0;
  std::uint64_t exceptions_sum = 0;
  for (int i = 0; i < values.size(); i += kBlockSize) {
    ...
    for (int j = 0; j < block_size; ++j) {
      ...
      if ((delta_nibbles & 1) && (j & 1)) {
        ...
        std::uint8_t last_byte = *(block->base() + block->length() - 1);
        block->RemoveLast(1);
  ...
  for (CellPoint cp : cell_points) {
    if (cp.level != level) {
      ...
      *have_exceptions = true;
      ...
      // std::uint64_t v = S2CellId::FromFaceIJ(cp.face, cp.si >> 1, cp.ti >> 1).
      //            parent(level).id() >> (2 * shift + 1);
      std::uint32_t sj = (((cp.face & 3) << 30) | (cp.si >> 1)) >> shift;
      std::uint32_t tj = (((cp.face & 4) << 29) | cp.ti) >> (shift + 1);
      std::uint64_t v = InterleaveUint32BitPairs(sj, tj);
...
std::uint64_t ChooseBase(const vector<std::uint64_t>& values, int level, bool have_exceptions,
  ...
  std::uint64_t v_min = kException, v_max = 0;
  for (auto v : values) {
    if (v != kException) {
      v_min = min(v_min, v);
      v_max = max(v_max, v);
  ...
  std::uint64_t base = v_min & ~BitMask(excluded_bits);
...
bool CanEncode(std::uint64_t d_min, std::uint64_t d_max, int delta_bits,
  ...
  std::uint64_t max_delta = BitMask(delta_bits);
  if (have_exceptions) {
...
BlockCode GetBlockCode(Span<const std::uint64_t> values, std::uint64_t base,
  ...
  std::uint64_t b_min = kException, b_max = 0;
  ...
  // chosen delta length.
  std::uint64_t max_delta = BitMask(delta_bits) - (have_exceptions ? kBlockSize : 0);
  int offset_bits = 0;
  if (b_max > max_delta) {
    ...
    std::uint64_t mask = BitMask(offset_shift);
    std::uint64_t min_offset = (b_max - max_delta + mask) & ~mask;
...
bool EncodedS2PointVector::InitCellIdsFormat(Decoder* decoder) {
  ...
  std::uint8_t header1 = decoder->get8();
  std::uint8_t header2 = decoder->get8();
...
S2Point EncodedS2PointVector::DecodeCellIdsFormat(int i) const {
  ...
  std::uint64_t offset = GetUintWithLength<std::uint64_t>(ptr, offset_bytes) << offset_shift;
  ptr += offset_bytes;
  ...
  std::uint64_t delta = GetUintWithLength<std::uint64_t>(delta_ptr, delta_bytes);
  ...
  std::uint64_t value = cell_ids_.base + offset + delta;
