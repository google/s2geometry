...
namespace gtl {
// Redefine these types in the same way they're defined in absl to avoid
// template parameter type deduction problems
template <size_t... Ints>
using IndexSequence = absl::integer_sequence<size_t, Ints...>;

template <typename... Ts>
using IndexSequenceFor = absl::make_index_sequence<sizeof...(Ts)>;

template <typename T, T... Ints>
using IntegerSequence = absl::integer_sequence<T, Ints...>;

template <size_t N>
using MakeIndexSequence = absl::make_integer_sequence<size_t, N>;

template <typename T, T N>
using MakeIntegerSequence = typename absl::utility_internal::Gen<T, N>::type;

template <typename T>
using in_place_type_t = absl::in_place_type_t<T>;
}  // namespace gtl
