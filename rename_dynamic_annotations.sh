#!/bin/bash
# https://chromium.googlesource.com/chromium/src/+/73.0.3648.1/third_party/abseil-cpp/rename_dynamic_annotations.sh
...
for w in \
  ...
  ValgrindSlowdown \
; do
  find src/s2/third_party/absl -type f -exec sed -i "s/\b$w\b/Absl$w/g" {} \;
done
for w in \
...
; do
  find src/s2/third_party/absl/ -type f -exec sed -i "s/\b$w\b/ABSL_$w/g" {} \;
done
